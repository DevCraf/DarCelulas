...
@section('content')
<div class="container">
  <div id="app" class="row justify-content-center align-items-center my-5 pt-5">
      ...
      <form @submit="checkForm" action="{!! route('postAgendaInscripcion') !!}" method="post">
        ...
        <template v-if="vives_bogota === 'Si'">
          ...
          <template v-if="has_donado === 'No'">
            ...
            <div class="form-group">
              <date-picker name="fecha_inscripcion" v-model="fecha_inscripcion_no" :config="options"></date-picker>
            </div>
          ...
          <template v-if="has_donado === 'Si'">
            ...
            <!-- DateTimePicker -->
            <div class="form-group">
              <date-picker name="ultima_donacion" v-model="ultima_donacion" :config="options_ultima_donacion"></date-picker>
            </div>
            ...
            <div v-show="frecuencia_donacion != null && ultima_donacion != null && tipo_donacion != null">
              <p><b>Elige a continuación la fecha y jornada en la cual puedes acercarte al Banco Distrital de Sangre - IDCBIS, para culminar tu proceso de inscripción en el Registro Nacional de Donantes de Células Progenitoras Hematopoyéticas<span class="text-danger"> *</span></b></p>
              <div class="form-group">
                <date-picker name="fecha_inscripcion" v-model="fecha_inscripcion_si" :config="options"></date-picker>
              </div>
            </div>
...
<script>
  ...
  function diferencia_fecha(fecha){
    ...
    if (validar < 1) {
    // Enviando variables
      app.options.defaultDate = hoy.format("YYYY-MM-DD");
      app.options.minDate = hoy.format("YYYY-MM-DD");
    }
    else{
    //   // Enviando variables
  ...
  var app = new Vue({
    ...
    methods: {
      ...
      c_tipo_donacion: function(e){
        // var parts = this.ultima_donacion.split('-');
        // var fecha = new Date(parts[2], parts[1] - 1, parts[0]);
        console.log(this.ultima_donacion);
        if (e.srcElement.value == "sangre_total") {
          // Sumando 4 meses
          fecha = moment(this.ultima_donacion).add(4, 'M');
        }
        else{
          // Sumando 2 meses
